...
#         DB_DIR = DB_DEV_DIR    
    try:
        db_file_path = DB_DIR + '/Hydat_20191016.sqlite3'
...
def get_daily_runoff(station):
    # create a database connection
    conn = create_connection()
    with conn:
        return select_daily_flows_by_station_ID(conn, station)
    conn.close()
...
def select_daily_flows_by_station_ID(conn, station):
    """
    Query tasks by priority
    :param conn: the Connection object
    :param station: station number (ID) according to WSC convention
...
#     df = pd.DataFrame(rows, columns=column_headers)
    ...
    id_var_headers = column_headers[:5] 
    ...
    out['DAILY_FLOW'] = df_flows['DAILY_FLOW']
